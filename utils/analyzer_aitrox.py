...
from mgamdata.dataset.Totalsegmentator.meta import CLASS_INDEX_MAP  # {name: index}
...
def draw_WinE_resp(resps: list[np.ndarray]):
    ...
    for i, ax in enumerate(axes.flat):
        img = resps[i][len(resps[i]) // 2][::-1]  # select the middle slice
    ...
    fig.tight_layout()
    fig.savefig(
        os.path.join(SAVE_ROOT, "temp_wine.png"),
        dpi=300,
    )
...
def draw_WinE_resp_distr(ori: np.ndarray, resps: list[np.ndarray]):
    ...
    fig.tight_layout()
    fig.savefig(
        os.path.join(SAVE_ROOT, "temp_wine_distr.png"),
        dpi=300,
    )
    plt.close(fig)
...
def draw_crsf_resp(inputs: np.ndarray, crsf_resp: np.ndarray, num_windows: int):
    ...
    fig.tight_layout()
    fig.savefig(
        os.path.join(SAVE_ROOT, "temp_crsf_resp.png"),
        dpi=300,
    )
    plt.close(fig)
...
def draw_confusion_matrix(gt: np.ndarray, pred: np.ndarray):
    ...
    fig.tight_layout()
    fig.savefig(
        os.path.join(SAVE_ROOT, "temp_cm.png"),
        dpi=500,
    )
    plt.close(fig)
...
if __name__ == "__main__":
    config_path = "/fileser51/zhangyiqin.sx/mmseg/AutoWindow/work_dirs/1.0.3.2.InstanceNorm/MedNeXt/MedNeXt.py"
    ckpt_path = "/fileser51/zhangyiqin.sx/mmseg/AutoWindow/work_dirs/1.0.3.2.InstanceNorm/MedNeXt/iter_500000.pth"
    # itk_path = "/home/zhangyq.sx/mgam_datasets/FLARE_2023_Data/spacing2_mha/"
    # itk_image_path = os.path.join(itk_path, "image", "1041.mha")
    # itk_mask_path = os.path.join(itk_path, "label", "1041.mha")
    
    # totalsegmentator dataset
    itk_path = "/home/zhangyq.sx/mgam_datasets/Totalsegmentator_Data/Totalsegmentator_dataset_v201/spacing2_mha/s1367/"
    itk_image_path = os.path.join(itk_path, "ct.mha")
    itk_mask_path = os.path.join(itk_path, "segmentations.mha")
    ...
    with torch.inference_mode():
        ...
        if hasattr(model, "pmwp"):
            ...
            crsf_resp = (
                model.pmwp.cross_window_fusion(all_window[:, None]).cpu().numpy().squeeze()
            )
    ...
    if wine_resps:
        ...
        draw_crsf_resp(all_window.cpu().numpy(), crsf_resp, num_windows)
